...
void XLA::lets_be_handsome(Mat &bgr_frame, Mat &gray_frame, Mat &topview_frame, Mat &binary_frame) {
    ...
    topview_frame = topview_frame(Rect(0, topview_frame.rows*5/8, topview_frame.cols, topview_frame.rows*3/8));
    ...
    filter_binary_image(gray_frame, binary_frame);
#ifdef SHOW_OUTPUT
...
*/    
    ...
    for (int i = 0; i < contours.size(); ++i) {
        ...
        if (get_average_intensity(gray_frame, contours[i]) < 127) {
            continue;
        }
...
*/
    ...
    if (abs(left_line.intersect(binary_frame.rows / 2) - right_line.intersect(binary_frame.rows / 2)) < 64) {
        res = split_line;
        res.is_left = split_line.intersect((int)binary_frame.rows * 3 / 4) < (binary_frame.cols / 2) ? 1 : 0;
        from_all = true;
    } else {
        ...
        if (min(sum_left, sum_right) / (sum_left + sum_right) < 0.2) {
            ...
            res.is_left = split_line.intersect((int)binary_frame.rows * 4 / 4) < (binary_frame.cols / 2) ? 1 : 0;
        } else {
            //both lane equal in length
            res.P = Point(0, (left_line.P.y + right_line.P.y) / 2);
            res.Q = Point(1e6, (left_line.Q.y + right_line.Q.y) / 2);
            res.calc_params();
            res.is_left = -1;
        }
    }
...
*/  
    ...
    if (current_line.is_left != -1) {
        ...
        //cerr << "multiplier = " << multiplier << ' ' << diff_dist << ' ' << diff_angle << ' ' << topview_width << endl;
    }
    /*
    double x = current_line.intersect(topview_height);
    if (current_line.is_left == 0) { //right
        if (x < topview_width / 2 * (6.0 / 5.0)) {
            plus = +10;
        }
    }
    if (current_line.is_left == 1) { //left
        if (x > topview_width / 2 * (4.0 / 5.0)) {
            plus = -10;
        }
    }
